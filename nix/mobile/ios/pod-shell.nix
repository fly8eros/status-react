  ...
  shellHook = ''
    pushd "$STATUS_REACT_HOME" > /dev/null
    {
      ...
      if diff -q ${podfileLock} ${manifestLock}; then
        ...
        (cd ios && pod install && sed -i -E 's/^LIBRARY_SEARCH_PATHS = \$\(inherited\)(.*secp256k1")$/LIBRARY_SEARCH_PATHS =\1 $(inherited)/g' 'Pods/Target Support Files/Pods-Status-StatusIm/Pods-Status-StatusIm.release.xcconfig')
      fi
    }
    popd > /dev/null
  '';
